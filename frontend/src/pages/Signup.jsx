import axios from "axios";
import { useState } from "react";
import { useNavigate, Link } from "react-router-dom";

export default function Signup() {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState({
    username: "",
    email: "",
    password: "",
  });

  const signup = async () => {
    if (loading) return;
    setLoading(true);

    try {
      
      const signupResponse = await axios.post("http://localhost:8080/auth/signup", data);

      
      if (signupResponse.status === 200 || signupResponse.status === 201) {
        await axios.post("http://localhost:8080/auth/request-otp", null, {
          params: {
            usernameOrEmail: data.username,
          },
        });

        navigate("/verify-otp", {
          state: { username: data.username },
        });
      }
    } catch (e) {
      console.error(e);
      // Extracts error message from backend if available, otherwise uses default
      const errorMessage = e.response?.data?.message || e.response?.data || "Signup failed. Please try again.";
      alert(errorMessage);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center font-sans px-4">
      {/* Top Navigation / Logo area */}
      <div className="absolute top-8 left-8 flex items-center gap-2">
        <div className="bg-purple-600 p-2 rounded-lg">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
        </div>
        <span className="text-xl font-bold tracking-tight">EncryptionFILE</span>
      </div>

      <div className="flex flex-col md:flex-row items-center justify-center max-w-6xl w-full gap-12">
        
        {/* Left Side: Branding Content */}
        <div className="md:w-1/2 text-center md:text-left">
          <h1 className="text-5xl md:text-7xl font-bold mb-4 tracking-tighter bg-gradient-to-r from-purple-400 to-pink-300 bg-clip-text text-transparent">
            SECURE YOUR <br /> FUTURE
          </h1>
          <p className="text-gray-400 text-lg max-w-md">
            As an award-winning encryption platform, we enable users to make their mark in history. Begin your secure journey with us.
          </p>
        </div>

        {/* Right Side: Signup Card */}
        <div className="md:w-1/2 w-full max-w-md">
          <div className="bg-gradient-to-br from-[#1a1333] to-[#0d0d0d] p-8 rounded-3xl border border-white/10 shadow-2xl">
            <h2 className="text-3xl font-bold mb-2">Create Account</h2>
            <p className="text-gray-400 text-sm mb-8">
              Join the secure cloud storage revolution.
            </p>

            <div className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Username</label>
                <input
                  disabled={loading}
                  className="w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 text-white placeholder-gray-600 transition-all disabled:opacity-50"
                  placeholder="Choose a username"
                  value={data.username}
                  onChange={(e) => setData({ ...data, username: e.target.value })}
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Email Address</label>
                <input
                  disabled={loading}
                  type="email"
                  className="w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 text-white placeholder-gray-600 transition-all disabled:opacity-50"
                  placeholder="name@example.com"
                  value={data.email}
                  onChange={(e) => setData({ ...data, email: e.target.value })}
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Password</label>
                <input
                  disabled={loading}
                  type="password"
                  className="w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 text-white placeholder-gray-600 transition-all disabled:opacity-50"
                  placeholder="••••••••"
                  value={data.password}
                  onChange={(e) => setData({ ...data, password: e.target.value })}
                />
              </div>

              <button
                onClick={signup}
                disabled={loading}
                className={`w-full mt-4 py-3 rounded-full font-bold transition-all transform active:scale-95 flex items-center justify-center gap-2 ${
                  loading 
                    ? "bg-gray-700 cursor-not-allowed text-gray-400" 
                    : "bg-white text-black hover:bg-gray-200"
                }`}
              >
                {loading ? (
                  <>
                    <span className="animate-pulse">PROCESSING...</span>
                  </>
                ) : (
                  "SIGN UP NOW"
                )}
              </button>

              <p className="text-center text-sm text-gray-400 mt-6">
                Already have an account?{" "}
                <Link to="/login" className="text-purple-400 font-semibold hover:underline">
                  Login here
                </Link>
              </p>
            </div>
          </div>
          
          <p className="text-center text-[10px] text-gray-600 mt-6 uppercase tracking-widest">
            By continuing, you agree to our Terms & Privacy Policy
          </p>
        </div>
      </div>
    </div>
  );
}